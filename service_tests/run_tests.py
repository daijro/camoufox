...
def generate_presets() -> dict:
    ...
    mac_per_context = [generate_context_fingerprint(os="macos") for _ in range(3)]
    print("  Generating 3 Linux per-context profiles...")
    linux_per_context = [generate_context_fingerprint(os="linux") for _ in range(3)]
...
def inject_timezone(ctx: dict, timezone_id: str) -> None:
    ctx["init_script"] = re.sub(
        r"w\.setTimezone\(Intl\.DateTimeFormat\(\)\.resolvedOptions\(\)\.timeZone\)",
        f"w.setTimezone({json.dumps(timezone_id)})",
        ctx["init_script"],
    )
    ctx["context_options"]["timezone_id"] = timezone_id
    ctx["config"]["timezone"] = timezone_id


# ─── Profile Config ───────────────────────────────────────────────────────────

def ctx_to_profile_config(ctx: dict, name: str, os_type: str) -> dict:
    ...
    return {
        ...
        "userAgent": config.get("navigator.userAgent", ""),
...
async def run_tests(
    ...
    # 3. Inject timezones
    all_ctxs = presets["macPerContext"] + presets["linuxPerContext"]
    for i, ctx in enumerate(all_ctxs):
        inject_timezone(ctx, TEST_TIMEZONES[i % len(TEST_TIMEZONES)])

    # 4. Build profile entries
    entries = []
    for i, ctx in enumerate(presets["macPerContext"]):
        entries.append({
            "ctx": ctx,
            "profile": ctx_to_profile_config(ctx, f"macOS Per-Context {chr(65 + i)}", "macos"),
        })
    for i, ctx in enumerate(presets["linuxPerContext"]):
        entries.append({
            "ctx": ctx,
            "profile": ctx_to_profile_config(ctx, f"Linux Per-Context {chr(65 + i)}", "linux"),
        })

    entries = entries[:max(1, min(profile_count, len(entries)))]
    ...
    # 6. Parse ff_version from browser_version specifier
    ff_version = None
        ...
        async with AsyncCamoufox(**launch_kwargs) as browser:
            ...
            for entry in entries:
                try:
                    ...
                    open_contexts.append({"context": context, "profile": entry["profile"]})
                except Exception as e:
                    pr = {"profile": entry["profile"], "results": None, "grade": "F",
