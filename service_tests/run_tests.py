...
WEBRTC_TEST_IP = "203.0.113.1"  # RFC 5737 TEST-NET, not a real IP
...
# ─── WebRTC Test Init Script ──────────────────────────────────────────────────

# Injected separately as test infrastructure (not a fingerprint attribute).
WEBRTC_INIT_SCRIPT = (
    f'if(typeof window.setWebRTCIPv4==="function")'
    f' window.setWebRTCIPv4({json.dumps(WEBRTC_TEST_IP)});'
)
...
async def run_tests(
    ...
    # 2. Build profile specs — fingerprints are generated by camoufox per-context
    all_specs = []
    for i in range(3):
    ...
    try:
        async with AsyncCamoufox(**launch_kwargs) as browser:
            ...
            for entry in entries:
                ...
                try:
                    context = await AsyncNewContext(browser, os=entry["os"], timezone_id=entry["timezone_id"])
                    await context.add_init_script(WEBRTC_INIT_SCRIPT)
